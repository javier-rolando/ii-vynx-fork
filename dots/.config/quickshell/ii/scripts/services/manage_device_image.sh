...
if [ "$ACTION" == "pick" ]; then
    # Pick a PNG or WebP file using kdialog or zenity
    if command -v kdialog >/dev/null; then
        kdialog --getopenfilename "$HOME" "*.png *.webp|Images (*.png *.webp)" 2>/dev/null
    elif command -v zenity >/dev/null; then
        zenity --file-selection --file-filter="*.png *.webp" 2>/dev/null
    else
