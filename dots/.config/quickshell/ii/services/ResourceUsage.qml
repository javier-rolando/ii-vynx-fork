...
Singleton {
	...
	Process {
		...
		command: ["bash", "-c", 
			...
			"gpu_stats=$(nvidia-smi --query-gpu=utilization.gpu,temperature.gpu --format=csv,noheader,nounits 2>/dev/null); " +
			"fi; " +
			"if [ -z \"$gpu_stats\" ] || [ \"$gpu_stats\" = \"0, 0\" ]; then " +
			"amd_path=\"\"; " +
		...
		running: root.gpuMonitoringEnabled
		stdout: SplitParser {
			onRead: data => {
				const parts = data.trim().split(/[\s,]+/)
				if (parts.length >= 2) {
